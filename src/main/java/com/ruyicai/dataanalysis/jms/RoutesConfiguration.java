			...
			camelContext.addRoutes(new RouteBuilder() {
				...
				public void configure() throws Exception {
					...
					from("jms:queue:VirtualTopicConsumers.dataanalysis.standardAvgUpdate?concurrentConsumers=20").to(
							"bean:standardAvgUpdateListener?method=process").routeId("足球-平均欧赔更新");
					from("jms:queue:VirtualTopicConsumers.dataanalysis.standardCacheUpdate?concurrentConsumers=20").to(
							"bean:standardCacheUpdateListener?method=process").routeId("足球-欧赔缓存更新");
