...
//			}
	...
	private void doProcess(Element match) {
		try{
			...
			String guestHalfScoreStr = match.elementTextTrim("bc2"); //客队上半场比分
			String home_Red = match.elementTextTrim("red1"); //主队红牌
			String guest_Red = match.elementTextTrim("red2"); //客队红牌
			String home_yellow = match.elementTextTrim("yellow1"); //主队黄牌
			String guest_Yellow = match.elementTextTrim("yellow2"); //客队黄牌
			String homeOrder = match.elementTextTrim("order1"); //主队排名
			String guestOrder = match.elementTextTrim("order2"); //客队排名
			...
			if (StringUtils.isNotBlank(matchTime)&&(oldMatchTime==null
					||!StringUtils.equals(convertMatchTime(matchTime), DateUtil.format("yyyy-MM-dd HH:mm:ss", oldMatchTime)))) {
				ismod = true;
				schedule.setMatchTime(DateUtil.parse("yyyy-MM-dd HH:mm:ss", convertMatchTime(matchTime)));
			}
			//上下半场开始时间
			Date oldMatchTime2 = schedule.getMatchTime2();
			if (StringUtils.isNotBlank(matchTime2)) {
				String pattern = "yyyy-MM-dd HH:mm:ss";
				Date matchTime2Date = DateUtil.parse(pattern, convertMatchTime(matchTime2));
